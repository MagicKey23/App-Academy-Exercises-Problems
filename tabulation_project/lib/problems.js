...
// Tabulation
//  let initial_step = nums[0];
//  let finished_step = new Array(nums.length + 1).fill(false);
//  let last = finished_step.length - 1;
//  finished_step[last] = true;
//  // If find step higher than previous step, replace the step
//     for(let i = 1; i <= initial_step;i++){
//         if (finished_step[i] == finished_step[last]) {return finished_step[last]; break;}
//         if(nums[i] > initial_step) initial_step = nums[i];
//     }
//     return false;
...
// Examples:
// 0 2 4
// maxNonAdjacentSum([2, 7, 9, 3, 0, 4])   // => 15, because 2 + 9 + 4
...
// to make the target amount. A coin value can be used multiple t1mes.
//
